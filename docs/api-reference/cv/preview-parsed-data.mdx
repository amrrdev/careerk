...
| `data.firstName`          | string | First name extracted from CV                                      |
| `data.lastName`           | string | Last name extracted from CV                                       |
| `data.cvEmail`            | string | Email extracted from CV                                           |
| `data.phone`              | string | Phone number extracted from CV                                    |
| `data.location`           | string | Location extracted from CV                                        |
| `data.linkedinUrl`        | string | LinkedIn URL extracted from CV                                    |
| `data.githubUrl`          | string | GitHub URL extracted from CV                                      |
| `data.portfolioUrl`       | string | Portfolio URL extracted from CV                                   |
...
| `data.skills`             | array  | Array of skills with `name` and `verified` flag                   |
| `data.workPreference`     | string | Work preference: ONSITE, REMOTE, HYBRID, ANY                      |
| `data.yearsOfExperience`  | number | Years of experience (from CV, cannot be modified)                 |
| `data.noticePeriod`       | number | Notice period in days                                             |
| `data.availabilityStatus` | string | Availability status: OPEN_TO_WORK, NOT_LOOKING, PASSIVELY_LOOKING |
...
<Info>
  **Skills Verification**: All skills returned from CV parsing are marked as `verified: true` since
  they were extracted from your uploaded CV document.
</Info>
