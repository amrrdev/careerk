...
const response = await fetch('http://localhost:3000/api/v1/auth/refresh-token', {
  ...
  credentials: 'include' // Required! Sends the refresh token cookie
});
...
response = session.post('http://localhost:3000/api/v1/auth/refresh-token')
...
<Steps>
  ...
  <Step title="Store New Refresh Token">
...
<AccordionGroup>
  ...
  <Accordion title="Before Expiration (Proactive)">
    Decode the JWT and refresh before it expires (e.g., 5 minutes before).
